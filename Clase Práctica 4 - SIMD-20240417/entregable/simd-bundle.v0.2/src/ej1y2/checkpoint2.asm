...
section .rodata: 	
align 16					;Alineamos antes de declarar en memoria
shuffle_h1: times 4 db 3 	;La mascara queda en posiciones contiguas de memoria , se lee al reves x el endianness
...
four_of_a_kind_asm:
	...
	shr ecx,2					; Divido por 4 / itero n/4 veces
...
.ciclo:
	...
	pand xmm0,xmm6		; xmm0:= 0V15 | 0V14 | .	.	.	| 0V1 | 0V0 -> En cada carta/byte nos queda solo el valor puesto que la rep. era [suit|value]

	;Pintamos c/mano del valor de la carta en el byte + significativo de esta
	...
	pcmpeqb xmm1,xmm0	; Comparo con los valores originales -> 0xff ff ff ff si las cartas de una mano tienen el mismo valor
	pcmpeqd xmm1,xmm5	; Comparo de a doublewords -> 0xffffffff si la mano cumple , todo 0 sino  
